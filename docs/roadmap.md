...
- [x] **#347 — a partner self-service surface.** `/__sandbox/*`, reachable with the `mfk_` key a partner already holds: their journal, inbox (with OTP extraction), resources and environment keys, plus the between-runs reset. A separate namespace rather than a loosened `/__admin` — ADR 0011 binds that surface to ignore sandbox keys entirely and a test asserts it, so this stands beside it and re-asserts the criterion from the new side. The tenant comes from the key and only from the key: there is no tenant header here, so cross-tenant access is not a check that could be wrong. Secret environment values are withheld, which is why #348 was taken first. Without `--sandbox-auth` the namespace is absent rather than open.
- [x] **#349 — edge hardening.** Three things an externally reachable host needs and a laptop does not, all verified absent before being built and all off by default: `--allow-outbound-host` (the hosts this instance may call — checked against the *rendered* webhook URL, since a template may be `{{request.headers.X-Callback}}`), `--max-request-body-bytes` with a per-tenant value clamped beneath it and a 413 naming which limit was hit, and `--allow-origin` (the admin API deliberately excluded, `/__sandbox` deliberately included). Building the first found that a proxy refusal would have propagated as an opaque 500 — the one proxy outcome the host can explain completely turned into the one that explains nothing. **Phase 1 of G22 is complete.**
- [x] **#353 — resource querying.** Filter, sort and field selection over a collection, on both the admin listing and the served `list`, sharing one evaluator so the sandbox and the screen watching it cannot disagree. The filter vocabulary is the dialect's own (`contains`, `matches`, `absent`) rather than a second one. `total` counts matches, numbers sort numerically, and a caller-supplied regex carries a timeout so a bad filter cannot take the host down. The primitive underneath — `IResourceStore.Find` — had already shipped with relations (#350), because scoping a relation *is* a field lookup. Writing the wire test surfaced a trap now documented: serve-time filtering needs `urlPath`, since `url` matches the query string too and would 404 the very request that wants filtering.
- [x] **#352 — values that resolve values.** Substitution walked the text once, so a host name had to be copied into every value that needed it — exactly the class of edit people get wrong. Values now compose, cycles are refused when a key is *saved* with the chain named rather than discovered as a hung request, and the depth bound is the backstop rather than the mechanism. Secrecy travels through composition, settled with the feature instead of after it, or `authHeader = Bearer {{apiToken}}` would have been a way around redaction. A constant finally says "this does not vary", and `--env key=value` gives every tenant a shared value it can still override. Mutation testing found the cycle search had to unmark a key on backtrack: without it, two values referencing the same third one — the ordinary shape — read as a loop.
- [x] **#358 — a retry does not create a second payment.** `Idempotency-Key` was ignored, so a partner's client library — which sends one and retries on timeouts — got two payments out of one, which is exactly the behaviour their production integration is built never to see. The same key with the same request now replays; the same key with a different one is a 409 rather than somebody else's answer; the fingerprint deliberately excludes headers, since a retry with a fresh trace id is the same request. Off by default and per tenant when on, because a suite testing double submission must not be quietly fixed by a host setting. The journal records both requests and marks the replay: hiding it would disagree with the client, and showing it as a fresh serve would claim the sandbox did the work twice.
- [x] **#357 — a tenant you can declare, suspend and offboard.** A tenant used to be *derived from owning a stub*, so a partner with a key and a full sandbox did not appear until they had one, and offboarding meant deleting things until nothing was left. Declaring is additive — the derived listing still answers exactly as before — and suspension refuses serving with the word *suspended* while deleting nothing, which is the state "paused pending payment" needed and never had. Deleting answers with a receipt rather than an "ok", and removes the declaration last so a failure leaves a repeatable operation instead of an unrecoverable half-deleted tenant. `--tenant-storage-limit` closes the bound nobody had: one document and one collection were capped, so a partner seeding a loop across many collections could still fill a shared host. Two things surfaced while building it: a replace at the ceiling was refused until the guard learned to subtract what it was replacing, and `ResourceOptions` was being registered twice, where the second call silently dropped the first one's number.
- [x] **#356 — what a consumer actually did.** `--usage` keeps bounded per-key counts — total, matched, unmatched, and each refusal separately, because a wrong credential, a spent quota and a scope refusal are three different conversations — plus the most-called paths with the unmatched ones tracked apart, since those are the integration going wrong and a busy path would crowd them out of a bounded table. Deliberately not a second journal: no headers, no bodies, no per-request timestamps, so journal masking cannot be walked around by reading usage. The path table is an approximate heavy-hitters counter and says so. A match is recorded rather than inferred, because a stub answering 404 and a call nothing models are opposite findings; and the serving path is instrumented by wrapping rather than at each of its dozen exits, since one of those would eventually be added without its counter.
- [x] **#355 — the life of a sandbox key.** Expiry, revocation as a state rather than a delete, rotation with an overlap, and a read-only scope — three optional fields whose defaults are exactly what a key issued before them meant, so every stored row on all four providers reads back unchanged. The refusals name themselves (expired and revoked say which; an unknown token still learns nothing), rotation is issue-deploy-lapse instead of an outage, and read-only is stated as a rule about the method rather than the effect, because a rule that needs a response template read is not one anybody can hold. Two things came out of building it: who revoked a key was silently tied to `--audit` being on, so an authenticated operator was recorded as `unknown`; and the dashboard rounded expiry to days, showing "in 1 d" for a 60-minute rotation overlap.
- [x] **#354 — quota that survives a second replica and a restart.** The per-key hourly quota was counted in the serving process, so behind two replicas a partner got their number per pod and a deploy refunded the hour. `IRateCounter` moves the count to Redis when one is configured (epoch-aligned buckets, so two hosts agree on which window an instant belongs to), and `--rate-burst <n>/<seconds>` adds a host-wide ceiling beside it that applies to keys with no quota too. Running it against two hosts and one Redis found two defects: a burst window an hour long shared the hourly quota's bucket and charged every request twice, and a key issued — or *revoked* — on one replica never reached the others, so API keys are now the fourth kind of state the change feed reconciles.
- [x] **#351 — named datasets.** A scenario across collections, declared once and loaded or reset in one call, with Faker reachable from a seed and a fixed seed making the data reproducible. Loading sorts parents first because integrity refuses a child whose parent does not exist yet; unloading reverses that because `restrict` refuses a parent while children exist. Atomicity is a compensating rollback, since integrity can only be checked against documents that exist. Seeding uses an ambient scope rather than Bogus's process-wide static, so a load cannot make concurrently served responses deterministic. Writing the tests found two defects: validation demanded a template be JSON *before* rendering, which refused every numeric helper, and the load handler asked for a `TimeProvider` the container does not register, so every load answered 500.
- [x] **#378 — embedding a related document.** `#350` shipped relations and then closed, leaving embedding tracked against an issue that no longer accepted work — the one failure the deferred-edge register cannot have — so it was re-filed and is now done. `?_expand=customer` on the served `read` and `list`, the admin read and the partner surface, sharing one parser and one handler so the four cannot describe a document differently. Bounded exactly as ADR 0015 said: one level, a declared relation named explicitly, parents only; depth is refused by the ordinary unknown-name rule rather than by a special case, so there is no query-planner code path waiting to be switched on. The parameter is `_expand` rather than the issue's `expand`, because every unprefixed parameter is already a field filter (#353) and claiming the bare word would take that field name from every document — and `_expand` is the spelling of the vocabulary ADR 0015 adopted. The parent lands in an envelope, since a top-level `customer` would be indistinguishable from a field the contract declares and would overwrite one that was. A missing parent embeds `null` rather than failing the read; an unknown name is a 400 naming what would have worked, resolved before the document is fetched so a typo cannot double as an id oracle. Mutation testing paid for itself in the refusal: the message was asserted only by substring, and the separator between several alternatives was never exercised at all.
